...
{
	if (ft_strncmp(line, ID_NORTH, ID_LEN_TEXTURE) == 0)
		return (parse_single_elem(ctx, ELEM_NO));
	if (ft_strncmp(line, ID_SOUTH, ID_LEN_TEXTURE) == 0)
		return (parse_single_elem(ctx, ELEM_SO));
	if (ft_strncmp(line, ID_WEST, ID_LEN_TEXTURE) == 0)
		return (parse_single_elem(ctx, ELEM_WE));
	if (ft_strncmp(line, ID_EAST, ID_LEN_TEXTURE) == 0)
...
{
	if (ft_strncmp(line, ID_FLOOR, ID_LEN_COLOR) == 0)
		return (parse_single_elem(ctx, ELEM_F));
	if (ft_strncmp(line, ID_CEILING, ID_LEN_COLOR) == 0)
