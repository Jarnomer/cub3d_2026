...
static void	draw_floor_pixels(t_game *game, t_floor *f, t_tex *tex, t_i32 y)
{
	t_i32	x;
	t_u32	color;
	t_u8	fog;
	...
	if (fog == 255)
		return ((void)fog_fill_row(game, y));
	x = 0;
	while (x < game->render.width)
	{
		color = fog_apply(sample_floor(tex, f->grid), fog);
...
void	render_floor_row(t_game *game, t_i32 y)
{
	...
	draw_floor_pixels(game, &f, tex, y);
}
